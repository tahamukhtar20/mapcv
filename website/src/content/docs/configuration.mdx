...
mapcv is driven by a single YAML config file. Use `mapcv init` to scaffold one with annotations, then edit it before running `mapcv generate`.

---

## `region`

Defines the geographic area to download in WGS-84 coordinates.
...
| `zoom`  | int   | XYZ tile zoom level. 15 ≈ 4 m/px, 17 ≈ 1.2 m/px, 19 ≈ 0.3 m/px |

---

## `tiles`

Controls how tiles are fetched. Exactly one of `source` or `url_template` is required.
...
<Aside type="caution">
Most sources serve 256 × 256 px tiles. Sources that serve 512 × 512 px tiles will produce patches at double the expected resolution.
</Aside>
...
| `label_field` | str    | `null`  | Property name used as the class label. IDs are assigned by encounter order starting at 1. `null` assigns class 1 to all polygons. |
